resource "digitalocean_droplet" "sn_node" {
    ...
    inline = [
      ...
      "echo \" node command is: sn_node --max-capacity $MAX_CAPACITY --root-dir --local-ip ${digitalocean_droplet.sn_node[count.index].ipv4_address} --external-ip ${digitalocean_droplet.sn_node[count.index].ipv4_address}  ~/node_data --hard-coded-contacts $HARD_CODED_CONTACTS -vvvvv &\"",
      "nohup 2>&1 ./sn_node --max-capacity $MAX_CAPACITY --root-dir --local-ip ${digitalocean_droplet.sn_node[count.index].ipv4_address} --external-ip ${digitalocean_droplet.sn_node[count.index].ipv4_address}  ~/node_data --hard-coded-contacts $HARD_CODED_CONTACTS -vvvvv &",
      "sleep 5",
