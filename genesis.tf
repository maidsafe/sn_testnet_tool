resource "digitalocean_droplet" "sn_genesis" {
    ...
    inline = [
      ...
      "nohup 2>&1 ./sn_node --first --local-ip ${digitalocean_droplet.sn_genesis.ipv4_address} --local-port=1314 --external-ip ${digitalocean_droplet.sn_genesis.ipv4_address} --external-port=1314 --root-dir ~/node_data -vvvvv &",
      "sleep 5;"
    ]
  }
