resource "digitalocean_droplet" "testnet_genesis" {
    ...
    provisioner "remote-exec" {
      inline = [
        ...
        "nohup heaptrack ./sn_node --first --local-addr ${digitalocean_droplet.testnet_genesis.ipv4_address}:${var.port} --skip-auto-port-forwarding --root-dir ~/node_data --log-dir ~/logs ${var.remote_log_level} &",
        "sleep 5",
